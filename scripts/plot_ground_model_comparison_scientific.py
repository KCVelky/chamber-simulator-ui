...
Run from the project root:
    python scripts/plot_ground_model_comparison.py --export-vector

Optional for the Matplotlib fallback:
...
UI_COLORS = {
    ...
    "source": "#ef4444",
    "microphone": "#2563eb",
    "estimate_1": "#22c55e",  # ground-aware estimate
    "estimate_2": "#f59e0b",  # free-field estimate
    "axis_text": "#334155",
    "label_text": "#0f172a",
    "grid": "#e8eef7",
    "legend_edge": "#cbd5e1",
...
def _method_error(results: dict, method: str) -> float:
    """Return the 3D error for a method, or NaN if unavailable."""
    try:
        return float(results["errors"][method]["error_3d_m"])
    except Exception:
        return float("nan")


def _legend_labels(results: dict) -> dict:
    """Centralize legend labels so PyVista and Matplotlib use identical wording."""
    e1 = _method_error(results, "Estimate 1 - with ground")
    e2 = _method_error(results, "Estimate 2 - without ground")
    return {
        "source": "True source",
        "microphone": "Microphones",
        "estimate_1": f"Estimate 1: ground-aware, $e_{{3D}}$ = {e1:.3f} m",
        "estimate_2": f"Estimate 2: free field, $e_{{3D}}$ = {e2:.3f} m",
    }


def _plain_legend_labels(results: dict) -> dict:
    """PyVista text does not render mathtext reliably, so keep this version plain."""
    e1 = _method_error(results, "Estimate 1 - with ground")
    e2 = _method_error(results, "Estimate 2 - without ground")
    return {
        "source": "True source",
        "microphone": "Microphones",
        "estimate_1": f"Estimate 1: ground-aware, e3D = {e1:.3f} m",
        "estimate_2": f"Estimate 2: free field, e3D = {e2:.3f} m",
    }


def _enable_pyvista_quality(plotter) -> None:
    """Enable optional PyVista/VTK quality features without breaking old versions."""
    for mode in ("ssaa", "fxaa", None):
        try:
            if mode is None:
...
def render_with_ui_pyvista(results: dict, args: argparse.Namespace, output: Path) -> bool:
    """
    Render with PyVista, with a publication-oriented layout.
    ...
    window_size = (int(args.width_px), int(args.height_px))
    plotter = pv.Plotter(off_screen=True, window_size=window_size)
    plotter.set_background(UI_COLORS["background"])
    _enable_pyvista_quality(plotter)
    ...
    plotter.add_mesh(room, style="wireframe", color=UI_COLORS["room"], line_width=1.5, name="room")

    # Transparent floor and back wall.
    ...
    for i, point in enumerate(clipped_mics):
        mic_mesh = pv.Sphere(radius=mic_radius, center=tuple(point), theta_resolution=48, phi_resolution=24)
        plotter.add_mesh(
            mic_mesh,
            color=UI_COLORS["microphone"],
            opacity=1.0,
            smooth_shading=True,
            specular=0.25,
            specular_power=18,
    ...
    est2_mesh = pv.Sphere(radius=estimate_radius, center=tuple(np.clip(est2, [0, 0, 0], [Lx, Ly, Lz])), theta_resolution=64, phi_resolution=32)
    plotter.add_mesh(
        est2_mesh,
        color=UI_COLORS["estimate_2"],
        opacity=0.96,
        smooth_shading=True,
        specular=0.35,
        specular_power=24,
        name="estimate_2_without_ground",
    )

    est1_mesh = pv.Sphere(radius=estimate_radius, center=tuple(np.clip(est1, [0, 0, 0], [Lx, Ly, Lz])), theta_resolution=64, phi_resolution=32)
    plotter.add_mesh(
        est1_mesh,
        color=UI_COLORS["estimate_1"],
        opacity=0.96,
        smooth_shading=True,
        specular=0.35,
        specular_power=24,
        name="estimate_1_with_ground",
    )

    source_mesh = pv.Sphere(radius=source_radius, center=tuple(np.clip(source, [0, 0, 0], [Lx, Ly, Lz])), theta_resolution=72, phi_resolution=36)
    plotter.add_mesh(
        source_mesh,
        color=UI_COLORS["source"],
        opacity=1.0,
        smooth_shading=True,
        specular=0.35,
        specular_power=30,
    ...
    if args.show_labels:
        _add_point_labels(plotter, np.asarray([est2]), ["Estimate 2"], point_color=UI_COLORS["estimate_2"], text_color=UI_COLORS["estimate_2"], font_size=14)
        _add_point_labels(plotter, np.asarray([est1]), ["Estimate 1"], point_color=UI_COLORS["estimate_1"], text_color=UI_COLORS["estimate_1"], font_size=14)
        _add_point_labels(plotter, np.asarray([source]), ["Source"], point_color=UI_COLORS["source"], text_color=UI_COLORS["label_text"], font_size=15)
    ...
    if args.title:
        plotter.add_text(args.title, position="upper_left", font_size=15, color=UI_COLORS["label_text"], font="arial")

    labels = _plain_legend_labels(results)
    try:
        plotter.add_legend(
            labels=[
                [labels["source"], UI_COLORS["source"]],
                [labels["microphone"], UI_COLORS["microphone"]],
                [labels["estimate_1"], UI_COLORS["estimate_1"]],
                [labels["estimate_2"], UI_COLORS["estimate_2"]],
            ],
            bcolor=UI_COLORS["legend_background"],
            border=True,
            size=(0.44, 0.18),
            loc=args.legend_loc,
            face="circle",
        ...
        # Very old PyVista versions may not support all keyword arguments.
        plotter.add_legend(
            labels=[
                [labels["source"], UI_COLORS["source"]],
                [labels["microphone"], UI_COLORS["microphone"]],
    ...
    plotter.screenshot(str(output), return_img=False, scale=int(args.supersample))
    plotter.close()
    return True
...
def _configure_matplotlib_for_publication() -> None:
    """Use SciencePlots when available, otherwise fall back to robust rcParams."""
...
def render_matplotlib_fallback(results: dict, args: argparse.Namespace, output: Path) -> None:
    ...
    _configure_matplotlib_for_publication()

    def draw_sphere(ax, center, radius, color, *, alpha=1.0, resolution=40, zorder=10):
        center = np.asarray(center, dtype=float).reshape(3)
        u = np.linspace(0, 2 * np.pi, resolution)
        v = np.linspace(0, np.pi, max(16, resolution // 2))
    ...
    def add_label(ax, xyz, text, color, dx=0.06, dy=0.06, dz=0.06, size=10):
        ...
        label.set_path_effects([pe.withStroke(linewidth=3.2, foreground="white")])
    ...
    def add_room(ax, room_dims):
        ...
        ax.add_collection3d(Line3DCollection(edges, colors=UI_COLORS["room"], linewidths=1.0, alpha=0.95))
    ...
    def add_floor(ax, room_dims):
        ...
        ax.add_collection3d(
            Poly3DCollection(
                verts,
                facecolors=UI_COLORS["floor"],
                edgecolors=UI_COLORS["floor_edge"],
                linewidths=0.8,
                alpha=0.50,
            )
        )
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    fig = plt.figure(figsize=(float(args.fig_width), float(args.fig_height)), facecolor="white", constrained_layout=True)
    ...
    for mic in mics:
        draw_sphere(ax, mic, args.mic_radius, UI_COLORS["microphone"], alpha=1.0, resolution=28, zorder=20)

    draw_sphere(ax, est2, args.estimate_radius, UI_COLORS["estimate_2"], alpha=0.96, resolution=42, zorder=40)
    draw_sphere(ax, est1, args.estimate_radius, UI_COLORS["estimate_1"], alpha=0.96, resolution=42, zorder=45)
    draw_sphere(ax, src, args.source_radius, UI_COLORS["source"], alpha=1.0, resolution=50, zorder=90)
    ...
    ticks = np.arange(0, max(room_dims) + 0.01, float(args.tick_step))
    ax.set_xticks(ticks[ticks <= Lx])
    ax.set_yticks(ticks[ticks <= Ly])
    ax.set_zticks(ticks[ticks <= Lz])

    for axis in (ax.xaxis, ax.yaxis, ax.zaxis):
        axis._axinfo["grid"]["color"] = UI_COLORS["grid"]
    ...
    labels = _legend_labels(results)
    legend_handles = [
        mlines.Line2D([], [], color=UI_COLORS["source"], marker="o", linestyle="None", markersize=9, label=labels["source"]),
        mlines.Line2D([], [], color=UI_COLORS["microphone"], marker="o", linestyle="None", markersize=7, label=labels["microphone"]),
        mlines.Line2D([], [], color=UI_COLORS["estimate_1"], marker="o", linestyle="None", markersize=8, label=labels["estimate_1"]),
        mlines.Line2D([], [], color=UI_COLORS["estimate_2"], marker="o", linestyle="None", markersize=8, label=labels["estimate_2"]),
    ]
    legend = ax.legend(
        ...
        frameon=True,
        fancybox=False,
        edgecolor=UI_COLORS["legend_edge"],
        facecolor=UI_COLORS["legend_background"],
        framealpha=0.96,
        borderpad=0.75,
        handletextpad=0.7,
    )
    ...
    fig.savefig(output, dpi=int(args.dpi), bbox_inches="tight", pad_inches=0.06)

    if args.export_vector:
        fig.savefig(output.with_suffix(".pdf"), bbox_inches="tight", pad_inches=0.06)
        fig.savefig(output.with_suffix(".svg"), bbox_inches="tight", pad_inches=0.06)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render the ground-model comparison as a publication-quality scientific figure.")
    parser.add_argument("--backend", choices=["auto", "pyvista", "matplotlib"], default="auto",
                        help="auto uses PyVista when available, then Matplotlib fallback.")
    ...
    parser.add_argument("--output", type=Path, default=PROJECT_ROOT / "outputs" / "ground_model_comparison_scientific.png")
    parser.add_argument("--dpi", type=int, default=350, help="Raster export resolution for Matplotlib.")
    parser.add_argument("--export-vector", action="store_true", help="Also export PDF and SVG when using Matplotlib.")
    parser.add_argument("--title", type=str, default="Semi-anechoic source localization: ground-aware vs free-field model")
    ...
    # Matplotlib camera and layout.
    parser.add_argument("--fig-width", type=float, default=9.4)
    parser.add_argument("--fig-height", type=float, default=7.7)
    parser.add_argument("--elev", type=float, default=24.0)
    parser.add_argument("--azim", type=float, default=-44.0)
    ...
    parser.add_argument("--height-px", type=int, default=1400)
    parser.add_argument("--supersample", type=int, default=2, help="Screenshot supersampling factor for PyVista.")
