...
ALGORITHMS = ["None", "TDOA", "ER_LS_FREE_FIELD", "ER_LS", "ER_NLS", "MLE_EM_GROUND"]
...
PARAMETER_HELP: Dict[Tuple[str, ...], Dict[str, str]] = {
    ...
    ("algorithms", "alg_choice"): {
        ...
        "effect": "TDOA exploite les retards. ER_LS_FREE_FIELD force le modèle 1/r² sans sol. ER_LS/ER_NLS peuvent utiliser le modèle sol/image si l’option sol est activée. MLE/EM utilise un modèle probabiliste avec source sol/image.",
    },
    ...
    def update_scene(self, cfg: Dict[str, Any], estimated_source: Any = None, comparison_estimates: Any = None, reset_camera: bool = False) -> None:
        ...
        if comparison_estimates:
            for idx, item in enumerate(comparison_estimates):
                ...
                self._plotter.add_point_labels(
                    np.asarray([estimated]), [label],
                    font_size=11, point_color=color, text_color="#0f172a",
                    shape_opacity=0.12, always_visible=True,
                )
...
class ComsolChamberSimUI(QMainWindow):
    ...
    def _tab_comparison(self) -> QWidget:
        ...
        intro = QLabel(
            ...
            "Tu peux comparer deux algorithmes avec la même famille de micros, ou deux familles de micros avec le même algorithme."
        )
        ...
        mic_a_group = QGroupBox("Famille de micros A")
        mic_a_lay = QVBoxLayout(mic_a_group)
        mic_a_help = QLabel("Une ligne par micro : x,y,z. Cette famille est utilisée pour la première estimation.")
        mic_a_help.setObjectName("FieldHelp")
        self.comparison_mics_a_text = QTextEdit()
        self.comparison_mics_a_text.setMinimumHeight(120)
        self.comparison_mics_a_text.setPlaceholderText("x,y,z\n1.000,1.000,1.200\n2.000,1.000,1.200")
        self.comparison_mics_a_text.textChanged.connect(self._on_config_changed)
        self._apply_parameter_tooltip(self.comparison_mics_a_text, ("comparison", "mics_a_csv"), "Famille de micros A")
        self.controls[("comparison", "mics_a_csv")] = self.comparison_mics_a_text
        mic_a_lay.addWidget(mic_a_help)
        mic_a_lay.addWidget(self.comparison_mics_a_text)
        lay.addWidget(mic_a_group)

        mic_b_group = QGroupBox("Famille de micros B")
        mic_b_lay = QVBoxLayout(mic_b_group)
        mic_b_help = QLabel("Une ligne par micro : x,y,z. Cette famille est utilisée pour la seconde estimation.")
        ...
        self.comparison_copy_a_button = QPushButton("Micros actuels → A")
        self.comparison_copy_a_button.setObjectName("SmallAction")
        self.comparison_copy_a_button.clicked.connect(lambda: self._copy_current_mics_to_comparison("A"))
        ...
        actions.addWidget(self.comparison_copy_a_button)
    ...
    def _on_config_changed(self) -> None:
        ...
        if self._auto_compute_enabled and not comparison_only:
    ...
    def _sync_comparison_controls(self, *_args: Any) -> None:
        ...
        for path, active in (
            ...
            (("comparison", "mics_a_csv"), mic_mode),
            (("comparison", "mics_b_csv"), mic_mode),
        ):
            widget = self.controls.get(path)
            if widget is not None:
                widget.setEnabled(enabled and active)
        for attr in ("comparison_run_button", "comparison_copy_a_button", "comparison_copy_b_button"):
            if hasattr(self, attr):
    ...
    def _copy_current_mics_to_comparison(self, target: str) -> None:
        try:
            ...
            if target.upper() == "A" and hasattr(self, "comparison_mics_a_text"):
                self.comparison_mics_a_text.setPlainText(text)
            elif target.upper() == "B" and hasattr(self, "comparison_mics_b_text"):
    ...
    def run_comparison(self) -> None:
        ...
        try:
            ...
            if mode == "Deux familles de micros":
                ...
                mics_a = str(comparison.get("mics_a_csv", "")).strip()
                mics_b = str(comparison.get("mics_b_csv", "")).strip()
                if len(parse_mics_csv(mics_a)) == 0 or len(parse_mics_csv(mics_b)) == 0:
                    raise ValueError("Les familles de microphones A et B doivent contenir au moins un point x,y,z.")
                ...
                configs = [("Famille A", cfg_a), ("Famille B", cfg_b)]
            else:
                ...
                configs = [(f"Algorithme A", cfg_a), (f"Algorithme B", cfg_b)]
